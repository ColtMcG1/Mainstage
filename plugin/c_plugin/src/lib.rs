...
fn compile_json(args_json: &serde_json::Value) -> String {
    ...
    // Inline compile implementation using helpers from `common`.
    fn candidate_compilers() -> Vec<&'static str> {
        #[cfg(target_os = "windows")] return vec!["cl", "gcc", "clang"];
        #[cfg(not(target_os = "windows"))] return vec!["gcc", "clang"];
    }
    ...
    let result = compile_sources_with(&sources, &flags, compiler.as_deref());
