...
use crate::semantic::builtin::{BUILTIN_FUNCS, BuiltinIntrinsic};
        ...
        AstType::Call { target, arguments } => {
            if let AstType::Member { target: recv, member } = &target.kind {
                if let AstType::Identifier { name: mname } = &member.kind {
                    let name_str = mname.as_ref();
                    if let Some(recv_slot) = lower_expr(cx, recv) {
                        match name_str {
                            "length" => {
                                let out = cx.temp();
                                cx.emit(Op::Length { target: out, array: recv_slot });
                                return Some(out);
                            }
                            "run" => {
                                // Treat receiver identifier as scope call
                                if let AstType::Identifier { name: scope_name } = &recv.kind {
                                    let f = cx.temp();
                                    cx.emit(Op::LoadConst {
                                        target: f,
                                        value: OpValue::Str(scope_name.as_ref().to_string()),
                                    });
                                    let sink = cx.temp();
            ...
            if let AstType::Identifier { name } = &target.kind {
                if let Some(def) = BUILTIN_FUNCS.get(name.as_ref()) {
                    match def.intrinsic {
                        ...
                        Some(BuiltinIntrinsic::Ask) => {
                            if let Some(arg) = arguments.get(0).and_then(|a| lower_expr(cx, a)) {
                                let out = cx.temp();
                                cx.emit(Op::Ask { question: arg, target: out });
                                return Some(out);
                            }
                            return None;
                        }
                        Some(BuiltinIntrinsic::Read) => {
                            if let Some(loc) = arguments.get(0).and_then(|a| lower_expr(cx, a)) {
                                let out = cx.temp();
                                cx.emit(Op::Read { location: loc, target: out });
                                return Some(out);
                            }
                            return None;
                        }
                        Some(BuiltinIntrinsic::Write) => {
                            if arguments.len() >= 2 {
                                if let (Some(loc), Some(content)) =
                                    (lower_expr(cx, &arguments[0]), lower_expr(cx, &arguments[1]))
                                {
                ...
                if cx.scope_names.contains(callee) {
                    ...
                    cx.emit(Op::LoadConst {
                        target: f,
                        value: OpValue::Str(callee.to_string()),
                    });
                    let mut args = Vec::new();
                    for a in arguments {
                        if let Some(s) = lower_expr(cx, a) {
                            args.push(s);
                        }
                    }
                    let ret = cx.temp();
