...
/// Represents assignment operators in the AST.
...
pub enum AstType<'a> {
    ...
    Stage {
        name: Cow<'a, str>,
        params: Vec<super::ast::AstNode<'a>>,
    },
    Task {
        name: Cow<'a, str>,
        params: Vec<super::ast::AstNode<'a>>,
    ...
    // Ternary statments are reduced to if/else
    IfElse {
        cond: Box<super::ast::AstNode<'a>>,
        if_body: Box<super::ast::AstNode<'a>>,
        else_body: Box<super::ast::AstNode<'a>>,
    },
    ...
    Array,
    Call {
        target: Box<super::ast::AstNode<'a>>,
        arguments: Vec<super::ast::AstNode<'a>>,
    },
    Member {
        target: Box<super::ast::AstNode<'a>>,
        member: Box<super::ast::AstNode<'a>>,
    },
    Index {
        target: Box<super::ast::AstNode<'a>>,
        index: Box<super::ast::AstNode<'a>>,
    },
    UnaryOp {
        op: UnaryOperator,
        expr: Box<super::ast::AstNode<'a>>,
        prefix: bool,
    },
    BinaryOp {
        op: BinaryOperator,
        left: Box<super::ast::AstNode<'a>>,
        right: Box<super::ast::AstNode<'a>>,
    },
...
impl<'a> AstType<'a> {
    ...
    pub fn into_lifetime(self) -> AstType<'static> {
        match self {
            ...
            AstType::Include { path } => AstType::Include {
            ...
            AstType::Workspace { name } => AstType::Workspace {
                name: Cow::Owned(name.into_owned()),
            },
            AstType::Project { name } => AstType::Project {
                name: Cow::Owned(name.into_owned()),
            },
            AstType::Stage { name, params } => AstType::Stage {
                name: Cow::Owned(name.into_owned()),
                params: params.into_iter().map(|p| p.into_lifetime()).collect(),
            },
            AstType::Task { name, params } => AstType::Task {
                name: Cow::Owned(name.into_owned()),
                params: params.into_iter().map(|p| p.into_lifetime()).collect(),
            },
            AstType::Block => AstType::Block,
            AstType::Assignment { op } => AstType::Assignment { op },
            ...
            AstType::Member { target, member } => AstType::Member {
                target: Box::new(target.into_lifetime()),
                member: Box::new(member.into_lifetime()),
            },
            AstType::Call { target, arguments } => AstType::Call {
                target: Box::new(target.into_lifetime()),
                arguments: arguments.into_iter().map(|a| a.into_lifetime()).collect(),
            },
            AstType::If { cond, body } => AstType::If {
            ...
            AstType::IfElse {
                ...
                cond: Box::new(cond.into_lifetime()),
                if_body: Box::new(if_body.into_lifetime()),
                else_body: Box::new(else_body.into_lifetime()),
            },
            ...
            AstType::Identifier { name } => AstType::Identifier {
                name: Cow::Owned(name.into_owned()),
            },
            AstType::ShellCmd { shell, command } => AstType::ShellCmd {
                shell: Cow::Owned(shell.into_owned()),
                command: Cow::Owned(command.into_owned()),
            },
            AstType::Str { value } => AstType::Str {
    ...
    pub fn into_owned(self) -> AstType<'static> {
        match self {
            ...
            AstType::Call { target, arguments } => AstType::Call {
                target: Box::new(target.into_owned()),
                arguments: arguments.into_iter().map(|a| a.into_owned()).collect(),
            },
            AstType::If { cond, body } => AstType::If {
            ...
            AstType::IfElse {
                ...
                cond: Box::new(cond.into_owned()),
                if_body: Box::new(if_body.into_owned()),
                else_body: Box::new(else_body.into_owned()),
            },
            AstType::Forto { init, limt, body } => AstType::Forto {
...
impl<'a> PartialEq for AstType<'a> {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (AstType::Assignment { op: o1 }, AstType::Assignment { op: o2 }) => o1 == o2,
            ...
            (AstType::While { cond: c1, body: b1 }, AstType::While { cond: c2, body: b2 }) => {
                c1 == c2 && b1 == b2
            }
            _ => false,
        }
