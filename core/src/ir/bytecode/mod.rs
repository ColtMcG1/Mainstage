...
pub fn emit_bytecode(module: &crate::ir::lower::IrModule) -> Vec<u8> {
	...
	for op in module.ops.iter() {
		match op {
			...
			IROp::Inc { dest } => { out.push(0x30); write_u32(&mut out, *dest as u32); }
			IROp::Dec { dest } => { out.push(0x31); write_u32(&mut out, *dest as u32); }
			IROp::Label { name } => {
				out.push(0x40);
				write_string(&mut out, name);
			}
			IROp::Jump { target } => { out.push(0x41); write_u32(&mut out, *target as u32); }
			IROp::BrTrue { cond, target } => { out.push(0x42); write_u32(&mut out, *cond as u32); write_u32(&mut out, *target as u32); }
			IROp::BrFalse { cond, target } => { out.push(0x43); write_u32(&mut out, *cond as u32); write_u32(&mut out, *target as u32); }
			IROp::Halt => { out.push(0x50); }
			IROp::AllocClosure { dest } => { out.push(0x60); write_u32(&mut out, *dest as u32); }
			IROp::CStore { closure, field, src } => { out.push(0x61); write_u32(&mut out, *closure as u32); write_u32(&mut out, *field as u32); write_u32(&mut out, *src as u32); }
			IROp::CLoad { dest, closure, field } => { out.push(0x62); write_u32(&mut out, *dest as u32); write_u32(&mut out, *closure as u32); write_u32(&mut out, *field as u32); }
			IROp::Call { dest, func, args } => {
				out.push(0x70);
				write_u32(&mut out, *dest as u32);
				write_u32(&mut out, *func as u32);
				write_u32(&mut out, args.len() as u32);
				for a in args.iter() { write_u32(&mut out, *a as u32); }
			}
			...
			IROp::Ret { src } => { out.push(0x80); write_u32(&mut out, *src as u32); }
		}
	}
...
fn write_value(out: &mut Vec<u8>, v: &Value) {
	match v {
		...
		Value::Array(arr) => {
			...
			for el in arr.iter() { write_value(out, el); }
		}
		Value::Null => { out.push(0x07); }
	}
}
